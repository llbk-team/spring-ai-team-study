...
springai.vision.downloadBase64Image = function(base64Src, filename) {
  const link = document.createElement('a'); // <a>를 생성 -> DOM 어딘가에는 부착
  link.href = base64Src; // <a href=""> 추가
  link.download = filename; // 
  document.body.appendChild(link); // DOM 어딘가에 추가
  link.click(); // 클릭 행위
  document.body.removeChild(link); // 추가했으면 제거
};
