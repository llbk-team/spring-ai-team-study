...
springai.printAnswerStreamText = async function (responseBody, targetId, chatPanelId) {
  const targetElement = document.getElementById(targetId); // span 태그
  const reader = responseBody.getReader();
  const decoder = new TextDecoder("utf-8");
  let content = "";
  while (true) {
    const { value, done } = await reader.read(); // value: 읽은 내용, done: true/false 더 이상 읽을 내용이 없으면 true
    if (done) break;
    let chunk = decoder.decode(value); //바이트 데이터를 문자열로 변환
    content += chunk; // 조각(chunk)을 이어붙여 전체 문장 구성
    // chunk가 완전한 HTML 태그 형태일 경우만 DOM 갱신
    if (!springai.isOpenTagIncomplete(chunk)) {
      let htmlContent = marked.parse(content); // HTML로 변환
      let styledContent = springai.applyBootstrapStyles(htmlContent);
      targetElement.innerHTML = styledContent;// innerHTML은 내부적으로 DOM 객체 생성(완전한 상태일 경우)
    }
    springai.scrollToHeight(chatPanelId);
  }
};
