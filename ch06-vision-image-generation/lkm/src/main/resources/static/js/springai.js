...
springai.printAnswerStreamText = async function (responseBody, targetId, chatPanelId) {
  ...
  /*------------------------------------------
  스트림으로 데이터 전달하여 모든 데이터가 전달되기 
  전까지 responseBody.text() 불가
  ------------------------------------------- */
  // 응답 본문 읽기
  ...
  while (true) {
    // 속성에 저장되는 값의 내용
    // value = 읽은 내용,
    // 응답 본문의 내용을 모두 읽었다면 done = true
    const { value, done } = await reader.read();
    if (done) break;

    // 읽은 내용을 다시 utf-8로 변환
    let chunk = decoder.decode(value);
    content += chunk; //누적시켜 완성

    // chunk가 완전한 태그인지 검사하여 DOM 객체로 생성
    if (!springai.isOpenTagIncomplete(chunk)) {
      // markdown -> html
      let htmlContent = marked.parse(content);
      let styledContent = springai.applyBootstrapStyles(htmlContent);
...
// ##### Bootstrap 스타일을 자동으로 적용하는 함수 #####
springai.applyBootstrapStyles = function (html) {
  // 임시 DOM 생성하여 HTML 조작
  const tempDiv = document.createElement("div");
  tempDiv.innerHTML = html;

  // 각 요소에 Bootstrap 클래스 적용
  tempDiv.querySelectorAll("h1, h2, h3, h4, h5, h6").forEach((el) => {
    el.classList.add("fw-bold", "text-primary", "mt-3", "mb-2");
  });

  tempDiv.querySelectorAll("p").forEach((el) => {
    el.classList.add("mb-3", "lh-base");
  });

  tempDiv.querySelectorAll("ul, ol").forEach((el) => {
    el.classList.add("ps-4", "mb-3");
  });
  ...
  tempDiv.querySelectorAll("blockquote").forEach((el) => {
    el.classList.add("blockquote", "ps-3", "border-start", "border-primary", "border-3", "text-muted", "fst-italic");
  });

  tempDiv.querySelectorAll("code").forEach((el) => {
    if (!el.parentElement.tagName === "PRE") {
      el.classList.add("bg-light", "text-danger", "px-1", "rounded");
    }
  });

  tempDiv.querySelectorAll("pre").forEach((el) => {
    el.classList.add("bg-dark", "text-light", "p-3", "rounded", "overflow-auto");
    const code = el.querySelector("code");
    if (code) {
      code.classList.remove("bg-light", "text-danger");
      code.classList.add("text-light");
    }
  });

  tempDiv.querySelectorAll("table").forEach((el) => {
    el.classList.add("table", "table-striped", "table-hover", "table-bordered", "mt-3");
  });

  tempDiv.querySelectorAll("a").forEach((el) => {
    el.classList.add("text-decoration-none", "fw-semibold");
  });
  ...
  tempDiv.querySelectorAll("em, i").forEach((el) => {
    el.classList.add("fst-italic", "text-muted");
  });
...
springai.vision.initCanvas = function (imgId, canvasId) {
  ...
  img.addEventListener("load", (e) => {
    springai.vision.resizeCanvas(imgId, canvasId);
  });
  ...
  const ctx = canvas.getContext("2d");  //붓 객체 얻어내기
...
springai.vision.resizeCanvas = function (imgId, canvasId) {
  ...
  // 캔버스 크기를 이미지의 실제 크기(랜더링되는 크기 X)와 동일하게 맞춤
  canvas.width = img.naturalWidth;
  canvas.height = img.naturalHeight;

  // 캔버스 화면 크기를 이미지의 화면 크기(랜더링되는 크기)와 맞춤
  const rect = img.getBoundingClientRect();
  canvas.style.width = rect.width + "px";
  canvas.style.height = rect.height + "px";

  // 캔버스 내부 버퍼 내용을 지움
  const ctx = canvas.getContext("2d");  // 2d로 그림
  ctx.clearRect(0, 0, canvas.width, canvas.height); // 지정된 범위만큼의 내부를 투명으로 지움
};
