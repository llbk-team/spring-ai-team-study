...
springai.voice.initMic = async function (handleVoice) {
  ...
  springai.voice.recognition = null;          // 음성 인식 객체 (사람의 목소리를 판단)
...
springai.voice.checkSilence = function () {
  // 분석 결과를 저장할 바이트 배열을 생성 (배열의 크기는 2048)
  // 양수만 저장
...
springai.voice.playAudioFormStreamingData = async function (response, audioPlayer) {
  try {
    ...
    // 스트림이 열리면 콜백되는 함수 등록
    // 미디어소스에 음성 데이터가 들어오면 response-mediaSource 스트림 열림
    mediaSource.addEventListener('sourceopen', async () => {
      ...
      while (true) {
        // 스트리밍 음성 데이터(청크) 읽기
        // 스트리밍되는 데이터가 있으면 done=false, 데이터가 더이상 없을 경우 done=true
