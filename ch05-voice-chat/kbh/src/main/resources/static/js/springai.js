...
springai.voice.initMic = async function (handleVoice) {
  ...
  springai.voice.mediaRecorder = null;        // 음성 녹음기 객체 (소리를 mp3로 저장)
  springai.voice.recognition = null;          // 음성 인식 객체 (사람 목소리 판별)
...
springai.voice.checkSilence = function () {
  ...
  const dataArray = new Uint8Array(springai.voice.analyser.fftSize); // 양수 저장
